<x-layout>
    ...
    <div class="p-1 flex flex-col flex-wrap gap-1 justify-center items-center">
        <h1 class="text-3xl font-bold  text-[#0b62db] mt-2.5 flex justify-center items-center">
            ASIAKKAANKATU 6* <div class="text-3xl rounded-sm flex justify-center items-center gap-1 text-[#0b62db]">
        ...
        <div class="w-full">
            ...
            @if (count($booked_slots) > 0)
                @foreach ($booked_slots as $date => $dayBookings)
                    @php
                        $slots = $dayBookings->pluck('timeslot')->sortBy('start_time')->values();
                        ...
                        $isFullDay = count($slots) === \App\Models\TimeSlot::count();
                    @endphp
                    <div class="w-full p-1 flex justify-between {{ $loop->index % 2 == 0 ? 'bg-gray-200' : '' }}">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                            stroke="currentColor" class="size-6" style="color:#254067">
                        ...
                        @if ($isFullDay)
                            <div class="font-bold text-green-700 flex-1 text-right mr-5">Entire Day</div>
                        @else
                            <div class="flex flex-wrap justify-end flex-1 gap-2">
                                @foreach ($merged as $range)
                                    <div class="">
                                        {{ substr($range[0], 0, 5) }} – {{ substr($range[1], 0, 5) }} Clubroom
                                    </div>
                                @endforeach
                            </div>
                        @endif
